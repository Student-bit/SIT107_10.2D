...
void loop() {
 ...
 if (digitalRead(3) == HIGH and activ == 0) {
    activ = 1;
    Serial.print("Bright system activated at ");
    Serial.println(millis());
    light_con();
    temp_con();
    hum_con();
    delay(20000);  //delay stops the activity switch being flipped multiple times from one motion
 }
 
 else if (digitalRead(3) == HIGH and activ == 1) {
    activ = 0;
    Serial.print("Bright system deactivated at ");
    Serial.println(millis());
    light_con();
    temp_con();
    hum_con();
